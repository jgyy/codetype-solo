...
let defaultReposCache: Repos | null = null;
function getDefaultRepos(): Repos {
    if (!defaultReposCache) defaultReposCache = composeRepos();
    return defaultReposCache;
}
...
export function httpAdapter<T>(
    h: Handler<T>,
    options?: { successStatus?: SuccessStatus<T>; repos?: Repos },
): ApiHandler {
    return async (event) => {
        const requestId = event.requestContext?.requestId ?? "";
        const caller = getCaller(event);
        const repos = options?.repos ?? getDefaultRepos();
        let result: Result<T, ApiError>;
        try {
            result = await h({ event, caller, requestId, repos });
        } catch (e) {
